...
impl<T: Config<I>, I: 'static> StoredMap<(T::AssetId, T::AccountId), T::Extra> for Pallet<T, I> {
  fn get(id_who: &(T::AssetId, T::AccountId)) -> T::Extra {
    ...
    if Account::<T, I>::contains_key(who, id) {
      Account::<T, I>::get(who, id).extra
    } else {
      Default::default()
    }
  }
  ...
  fn try_mutate_exists<R, E: From<DispatchError>>(
    ...
    let &(id, ref who) = id_who;
    let mut maybe_extra = Some(Account::<T, I>::get(who, id).extra);
    ...
    // Otherwise, we fail.
    Account::<T, I>::try_mutate_exists(who, id, |maybe_account| {
