...
# chainspec defaults to testnet-local if no arguments are passed to this script;
# if arguments are passed in, the first is the chainspec
chainspec="${1:-testnet-local}"
...
tidechain="target/release/tidechain"

# ensure the tidechain binary exists and is up to date
if [ ! -x "$tidechain" ] || [ "$tidechain" -ot "$last_modified_rust_file" ]; then
  cargo build --release
fi
...
# The name must be one of those which can be passed to the tidechain binary, in un-flagged form,
# one of:
