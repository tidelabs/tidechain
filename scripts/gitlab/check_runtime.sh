...
# Check for any changes in any runtime directories (e.g., ^runtime) as
...
SUBSTRATE_REPO="https://github.com/tidelabs/substrate"
...
boldprint "make sure we have all tags"
...
boldprint "make sure the dev branch is available in shallow clones"
git fetch --depth="${GIT_DEPTH:-100}" origin dev
...
please make sure versions are bumped in tidechain accordingly
EOT
